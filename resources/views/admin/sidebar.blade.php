...
<aside x-data="{ crudOpen: {{ request()->is('jobs*') || request()->is('berita*') ? 'true' : 'false' }} }" class="flex flex-col w-64 h-full bg-white shadow-lg">
        ...
        <div>
            <button @click="crudOpen = !crudOpen"
                ...
                <svg xmlns="http://www.w3.org/2000/svg"
                    ...
                    :class="{ 'rotate-90': crudOpen }" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />
                </svg>
            </button>

            <!-- Submenu -->
            <div x-show="crudOpen" x-transition:enter="transition ease-out duration-200"
                x-transition:enter-start="opacity-0 -translate-y-2" x-transition:enter-end="opacity-100 translate-y-0"
                x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100"
                x-transition:leave-end="opacity-0" class="mt-2 ml-10 space-y-2">
        ...
        <form method="POST" action="{{ route('logout') }}">
            ...
            <button type="submit"
                ...
                <span class="relative">
                    Logout
                    <span
                        class="absolute bottom-0 left-0 w-full h-px transition-transform duration-200 ease-out transform scale-x-0 bg-yellow-600 group-hover:scale-x-100"></span>
                </span>
            </button>
        </form>
    </div>
