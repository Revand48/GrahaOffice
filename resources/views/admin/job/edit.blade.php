...
<div class="max-w-3xl mx-auto">
    ...
    <form action="{{ route('job.update', $job->id) }}" method="POST" class="space-y-4 bg-white p-6 rounded shadow">
        @csrf
        @method('PUT')

        <div>
            <label class="block font-medium mb-1">Posisi <span class="text-red-500">*</span></label>
            <input type="text" name="posisi" value="{{ old('posisi', $job->posisi) }}" class="w-full border p-2 rounded" required>
        </div>

        <div class="grid grid-cols-2 gap-4">
            <div>
                <label class="block font-medium mb-1">Jumlah Lowongan <span class="text-red-500">*</span></label>
                <input type="number" name="jumlah" value="{{ old('jumlah', $job->jumlah) }}" min="1" class="w-full border p-2 rounded" required>
            </div>
            ...
            <div>
                ...
                <select name="tipe" class="w-full border p-2 rounded" required>
                    ...
                    <option value="Fulltime" {{ old('tipe', $job->tipe) == 'Fulltime' ? 'selected' : '' }}>Fulltime</option>
                    <option value="Internship" {{ old('tipe', $job->tipe) == 'Internship' ? 'selected' : '' }}>Internship</option>
                </select>
            </div>
        </div>
        ...
        <div>
            ...
            <input type="text" name="lokasi" value="{{ old('lokasi', $job->lokasi) }}" class="w-full border p-2 rounded" required>
        </div>

        <div>
            <label class="block font-medium mb-1">Detail / Syarat (masukkan tiap syarat di baris baru)</label>
            <textarea name="detail" rows="6" class="w-full border p-2 rounded">{{ old('detail', $job->detail) }}</textarea>
